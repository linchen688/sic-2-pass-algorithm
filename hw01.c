...
char* lookup (char *s) {//what is input ?
    int i = 0;
    int nf = 1;
    while ((i < MAXOP) && (nf)) {
...
void pass1 () {
    int read_check = readline();

    //get the start address
    if(strcmp(a_start,op)==0)
    {
    ...
    while(strcmp(a_end,op)!=0)
    {
        ...
        if(read_check!=0){ 
            ...
            if(strcmp(label,"      ")!=0){
                ...
                //check symbol table is there exist currrent label
                if(new_search(label)==0){
                    symtab = insert(label,locctr);
                }else{
                    //already exist the current suymbol
                    printf("duplicate symbol\n");
                }
            }

            //check optable is there exist current opcode
            if(lookup(op)!=NULL){
                locctr+=3;
            }else if( strcmp(op,a_word)==0){
                locctr+=3;//one word = 3 byte
            }else if (strcmp(op,a_resw)==0){
                locctr+=(3*atoi(operand));//reserv word = 3byte * "how many words"
            }else if(strcmp(op,a_resb)==0){
                locctr+=atoi(operand);//reserv byte = add "a mount of bytes"
            }else if (strcmp(op,a_byte)==0){
                locctr+=operand_len();//reserv char
            }else{
